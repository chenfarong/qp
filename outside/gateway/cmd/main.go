...
func main() {
	...
	logger.Infof("监听地址: %s", config.AppConfig.Gateway.Host)
	logger.Infof("WebSocket端口: %d", config.AppConfig.Gateway.WsPort)
	logger.Infof("gRPC端口: %d", config.AppConfig.Gateway.GrpcPort)
	logger.Infof("最大连接数: %d", config.AppConfig.Gateway.MaxConnections)
	logger.Infof("会话超时时间: %d秒", config.AppConfig.Gateway.SessionTimeout)
	logger.Info("========================================================")
