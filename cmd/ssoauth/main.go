...
func main() {
	...
	// 初始化日志客户端
	logClient, err := logger.NewClient(fmt.Sprintf("localhost:%d", config.Server.Logger.UdpPort), fmt.Sprintf("ssoauth://localhost:%d", config.Server.Ssoauth.Port))
	if err != nil {
		log.Printf("Warning: Failed to initialize log client: %v", err)
	} else {
		defer logClient.Close()
		// 发送测试日志
		logClient.Warn("SSO Auth server starting")
	}
	...
	go func() {
		defer recoverPanic(logClient, "SSO Auth gRPC")
		startGRPCServer(authService, grpcPort)
	}()
	...
	// 向gateway注册协议编号段
	go func() {
		defer recoverPanic(logClient, "SSO Auth Gateway Registration")
		registerToGateway("ssoauth", fmt.Sprintf("localhost:%d", config.Server.Ssoauth.Port), 1, 100)
	}()
	...
	defer recoverPanic(logClient, "SSO Auth HTTP")
